...
PATIENT_STUDIES_URL = r'/api/patients/(\d+)/s'
...
SERIES_PROCESS_URL = r'/api/series/(\d+)/process/by_plugin/(\d+)/image'
...
INSTANCE_CONVERT_URL = r'/api/instances/(\d+)/limit/(\d+)/mid/((\-?)\d+)'
DCM_UPLOAD_URL = r'/api/dcm/upload'
